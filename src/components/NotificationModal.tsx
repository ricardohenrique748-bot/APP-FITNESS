      ...
      <div className="w-full max-w-md bg-[#1e2024] border border-[#282a2e] rounded-2xl p-5 shadow-2xl flex flex-col gap-4 max-h-[85vh] overflow-y-auto no-scrollbar">
        <div className="flex items-center justify-between border-b border-[#282a2e] pb-3">
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-[#c5f400] text-[22px]">notifications</span>
            <h3 className="text-[18px] font-bold text-white">Alertas & Diagnóstico GoFit</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-col gap-3">
          {notifications.map((item) => (
            <div
              key={item.id}
              className="flex items-start gap-3 p-3 rounded-xl bg-[#1a1c20] border border-[#282a2e]/50"
            >
              <div className="w-8 h-8 rounded-lg bg-[#282a2e] flex items-center justify-center shrink-0">
              ...
              <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-[#8e9379]">{item.time}</span>
                </div>
                <p className="text-[12px] text-[#c2c6d2] leading-relaxed">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <button
          onClick={onClose}
          className="w-full py-2.5 rounded-full bg-[#282a2e] hover:bg-[#333539] text-white font-semibold text-[13px] transition-colors mt-1"
        >
          Entendido
        </button>
      </div>
